
## 👋 Hi, I'm Ajay Agrawal

**AI-Powered Automation & Infrastructure Specialist** | Python Developer | Open Source Enthusiast

---

### 🎯 About Me
I'm passionate about building **intelligent automation systems** that solve real-world problems. 
Combining **AI/ML** with **DevOps** to create self-healing infrastructure and developer tools 
that scale. Currently working as an **Automation Framework Developer** at a leading MNC.

---

### 🚀 Featured Projects

| Project | Description | Tech | Links |
|---------|-------------|------|-------|
| **NeurOps** | AI-driven system that detects, predicts, and heals infrastructure failures | Python, Gemini, Google Cloud, Kubernetes | [🔗](https://github.com/ajayagrawalgit/NeurOps) |
| **MickTrace** | Production-ready async logging library with cloud integration | Python, AWS/Azure/GCP | [🔗](https://github.com/ajayagrawalgit/MickTrace) |
| **Apex** | Multi-agent malicious URL detection with Gemini AI | Python, Google ADK, Gemini | [🔗](https://github.com/ajayagrawalgit/Apex) |
| **IntentOS** | Gemini-powered data transformation engine | Python, NLP, Data Processing | [🔗](https://github.com/ajayagrawalgit/IntentOS) |

---

### 🛠️ Skills & Expertise

**Core:**
- 🐍 **Python** (Automation, ML, CLI tools)
- 🔧 **DevOps & Automation** (Puppet, Ansible, Infrastructure as Code)
- ☁️ **Cloud Platforms** (AWS, Azure, GCP)
- 🤖 **AI/ML** (Gemini, LangFlow, LLM Agents)

**Specializations:**
- Agentic AI & Self-Healing Systems
- Infrastructure Automation & Configuration Management
- Production-Grade Logging & Observability
- Security Tools (Phishing Detection, Password Security)

---

### 📚 Currently Learning
- 🔍 Advanced LLM agents, Kubernetes orchestration, and building production-grade AI systems. 
- Diving deeper into chaos engineering and self-healing infrastructure patterns.

### 🤝 Available For
Collaborations on AI-driven DevOps tools, open-source automation frameworks, and 
production-ready infrastructure solutions. Love hackathons, SRE discussions, and building tools 
that matter.

### 🎖️ Holopin Board
[![Ajay's Holopin Badges](https://holopin.me/ajayagrawalgit)](https://holopin.io/@ajayagrawalgit)

---

### 📬 Let's Connect
- 📧 Email: [ajayagrawalofficial@gmail.com](mailto:ajayagrawalofficial@gmail.com)
- 💼 LinkedIn: [theajayagrawal](https://www.linkedin.com/in/theajayagrawal/)

---

**"Building tomorrow's infrastructure, today."** 🚀
